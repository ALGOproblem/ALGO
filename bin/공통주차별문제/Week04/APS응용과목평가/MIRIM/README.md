...
## 문제 요약
### 집과 고양이
고양이는 모두 자신의 집을 1개씩 소유   
집을 통해 건물주 고양이의 털 1개의 색상을 알 수 있음.   
고양이는 각자 1개 이상의 털 색을 가짐   
N개의 집에 대해서 건물주를 추측하는 모든 경우의 수를 출력하라.   

## 문제 접근
### 순열
- N마리의 고양이는 N개의 집과 모두 연결되어야 한다.
- N마리와 N개의 집은 모두 구분된다.
- n번째 고양이가 k번째 집에 배정되는 것이 구분 가능하다 = 순서가 있다.
- 따라서 조건이 있는 순열이라고 이해할 수 있다.

### 선택권
- 값이 오직 하나인 집이 고양이를 선택한다고 하자.

## 문제 풀이
### 1. 자료 구조
- 집: `int[] home`, 입력 시 `-'a'`를 해주어 입력값으로 받게 한다.
	- `home[i] = sc.next().charAt(0) - 'a';`
- 고양이: `int[][] cats`로 입력받아 고양이마다 배열크기를 다르게 지정
- 집-고양이: `int[] catOrder`를 통해 해당 집마다 배정된 고양이의 인덱스를 저장한다.
- 이미 건물주: `boolean[] selected`를 통해 이미 집에 배정된 고양이인지 확인한다.

### 2. 순열
- 종료조건: `homeIdx`에 대하여 N이 되면 종료
- for문을 통해 0 ~ N - 1까지 루프하며 수행한다.
- (1) 이미 배정된 고양이면 다음 고양이를 확인
- (2) 고양이의 털을 확인해 포함되어 있다면 배정 -> 다음 집으로 넘어가기
- (3) 원상복귀: 해당 고양이를 배정하지 않고 다음 고양이를 확인한다
